...
export default function RankingBoard({ rankings, editable = false }: { rankings: any[]; editable?: boolean }) {
  ...
  function getCsrfToken() {
    return document.cookie.split("; ").find((row) => row.startsWith("admin-csrf-token="))?.split("=")[1];
  }
  ...
  function handleDragEnd(event: any) {
    ...
    const oldIndex = myRankings.findIndex((p) => p.team === active.id);
    const newIndex = myRankings.findIndex((p) => p.team === over.id);
  ...
  return (
    <div className="team-ranking-board">
      <div className="team-ranking-search">
        <input type="text" placeholder="Search team" value={search} onChange={(e) => setSearch(e.target.value)} />
      </div>

      <div className="team-ranking-grid">
        <div>
          <h2>ESPN Consensus</h2>
          {filteredConsensus.map((team: any) => (
            <div className="team-ranking-card" key={team.team}>
              <div className="team-ranking-rank">{team.consensusRank}</div>
        ...
        <div className="team-ranking-delta-column">
          <h2>Δ</h2>
          {filteredMyRankings.map((team: any) => {
            const delta = team.consensusRank - team.myRank;
            return <div className={`team-ranking-delta ${delta > 0 ? "up" : delta < 0 ? "down" : "flat"}`} key={team.team}>{delta === 0 ? "—" : delta > 0 ? `⬆ +${delta}` : `⬇ ${Math.abs(delta)}`}</div>;
          })}
        </div>
        ...
        <div>
          ...
          <DndContext collisionDetection={closestCenter} onDragEnd={handleDragEnd}>
            <SortableContext items={filteredMyRankings.map((p) => p.team)} strategy={verticalListSortingStrategy}>
              {filteredMyRankings.map((team: any) => <SortableTeam key={team.team} team={team} locked={locked} />)}
            </SortableContext>
          </DndContext>
        </div>
      </div>
      ...
      {!locked && (
        <div className="team-ranking-actions">
          <button className="team-ranking-save" onClick={saveRankings}>Save Rankings</button>
          <button className="team-ranking-finalize" onClick={finalizeRankings}>🏆 Finalize Rankings</button>
        </div>
      )}
    </div>
  );
